...
export default function myBlogs() {
  ...
  useEffect(() => {
    const alldata = async () => {
      try {
        ...
        setBlogs(response.data.data); // 데이터는 response.data 안에 들어있습니다.
      } catch (e) {
        console.error(e);
      }
    };
  ...
  function ExploreNFT() {
    // 구매 가능한 NFT 목록들 가져온 페이지로
    window.open("ExploreNFT");
  }

  function seeMyNFT() {
    // 내가 보유하고 있는 NFT 목록 페이지
  ...
  return (
    <div className={styles.myBlogscontainer}>
      ...
      <div>
        ...
        <button className={styles.button_see_my_NFT} onClick={hisBlogs}>
          hisBlogs
        </button>
      </div>
