...
export default function newStory() {
 ...
 const [text, setText] = useState("");
   ...
   useEffect(() => { // 로컬 함수 이외의 외부 함수 ( 서버 ) 로부터 데이터 받아와서 동기화 시키는 것이므로 side effect 발생하기에 useEffect 사용
    axios.get("서버 측 로그인 구현한 js 주소").then((res) => {
      if(res.status === 200 && res.data.name) { // 로그인 상태라면 터미널에 출력하고 페이지 접근 허용
 ...
 /*  // 2. 글 작성 완료하고 publish 버튼 누르면 서버단으로 post 전송. 
  ...
  const publish = async () => {
   ...
   await axios
     .post("서버 주소", { // 서버로 id와 글 제목, 내용을 같이 전달
       userId: userInfo._id,
       title: title,
       text : text
     })
